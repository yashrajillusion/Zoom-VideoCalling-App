...
export const initializeListensers = async (userId) => {
  ...
  currentUserRef.child("offers").on("child_added", async (snapshot) => {
    ...
    if (data?.offer) {
      // console.log(data.offer.userId);
      // console.log(store.getState().user.participants[`${data.offer.userId}`]);
      const pc =
        store.getState().user.participants[`${data.offer.userId}`]
          .peerConnection;
  ...
  currentUserRef.child("offerCandidates").on("child_added", (snapshot) => {
    ...
    if (data.userId) {
      console.log();
      const pc = store.getState().user.participants[data.userId].peerConnection;
  ...
  currentUserRef.child("answers").on("child_added", (snapshot) => {
    ...
    if (data?.answer) {
      const pc =
        store.getState().user.participants[data.answer.userId].peerConnection;
...
const createAnswer = async (otherUserId, userId) => {
  const pc = store.getState().user.participants[otherUserId].peerConnection;
